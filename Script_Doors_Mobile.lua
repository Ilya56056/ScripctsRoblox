...
local NotificationCoroutine = coroutine.create(function()
    LatestRoom.Changed:Connect(function()
        if GUI.Flags["BookESP"].Value == true then
            if LatestRoom.Value == 50 then
                coroutine.resume(BookCoroutine)
            end
        end
        if GUI.Flags["FigureToggle"].Value == true then
            if LatestRoom.Value == 50 then
    ...
    workspace.ChildAdded:Connect(function(inst)
        if inst.Name == "RushMoving" and GUI.Flags["MobSpawn"].Value == true then
            if GUI.Flags["AvoidRush"].Value == true then
        ...
        elseif inst.Name == "AmbushMoving" and GUI.Flags["MobSpawn"].Value == true then
            if GUI.Flags["AvoidRush"].Value == true then
                GUI:MakeNotification({
                    Name = "Warning!",
